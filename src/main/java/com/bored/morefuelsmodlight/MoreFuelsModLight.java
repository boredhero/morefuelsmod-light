...
@Mod(modid = MoreFuelsModLight.MODID, version = MoreFuelsModLight.VERSION, name = MoreFuelsModLight.MODNAME, acceptedMinecraftVersions = "[1.6.1,1.6.4]")
public class MoreFuelsModLight
{
    ...
    public void preinit(FMLPreInitializationEvent event){
		...
		boolean enableFlowers = config.get(Configuration.CATEGORY_GENERAL, "fuelenableFlowers", true).getBoolean(true);
		if(enableFlowers)
			GameRegistry.registerFuelHandler(new Flowers());
		boolean enableGrass = config.get(Configuration.CATEGORY_GENERAL, "fuelenableGrass", true).getBoolean(true);
		if(enableGrass)
			GameRegistry.registerFuelHandler(new Grass());
